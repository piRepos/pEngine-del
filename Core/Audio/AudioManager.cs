...
using pEngine.Core.Audio.Mixing;

namespace pEngine.Core.Audio
{
    public delegate void AudioDevicesChangedEventHandler(List<AudioDevice> CurrentDevices);

    public class AudioManager : IDisposable, IUpdatable
    {
		pEngine gameHost;

        /// <summary>
        /// Initializes a new instance of the <see cref="pEngine.Audio.AudioManager"/> class.
        /// </summary>
        public AudioManager(pEngine host)
        {
			gameHost = host;
        ...
        public void Initialize()
        {
			...
			gameHost.Debug.Log($"BASS initialized");
        }
        ...
        /// <summary>
        /// Releases all resource used by the <see cref="pEngine.Audio.AudioManager"/> object.
        /// </summary>
        /// <remarks>Call <see cref="Dispose"/> when you are finished using the <see cref="pEngine.Audio.AudioManager"/>. The
        /// <see cref="Dispose"/> method leaves the <see cref="pEngine.Audio.AudioManager"/> in an unusable state. After
        /// calling <see cref="Dispose"/>, you must release all references to the <see cref="pEngine.Audio.AudioManager"/> so
        /// the garbage collector can reclaim the memory that the <see cref="pEngine.Audio.AudioManager"/> was occupying.</remarks>
        public void Dispose()
        {
            Bass.Free();
        }
