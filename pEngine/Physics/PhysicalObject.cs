...
{
	/// <summary>
	/// Base class for all objects that have a transformation.
	/// </summary>
	public abstract partial class PhysicalObject : GameObject
	{
		/// <summary>
		...
		protected override void OnUpdate(IFrameBasedClock clock)
		{
			base.Update(clock);
		...
		public Vector2 PercentileVecByParent(double percent)
		{
			...
			if (!(Parent is PhysicalObject))
				throw new InvalidOperationException("Parent is not a PhysicalObject object.");

			return PercentileVecByOtherObject(percent, Parent as PhysicalObject);
		}
		...
		public Vector2i ToParentSpace(Vector2i input)
		{
			...
			return ToSpaceOfOtherObject(input, Parent as PhysicalObject);
		}
