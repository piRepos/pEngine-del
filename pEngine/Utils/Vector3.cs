...
namespace pEngine
{
	...
	public struct Vector3 : IEquatable<Vector3>
	{
		...
		{
			get { return 1.0f / MathHelpers.InverseSqrtFast(X * X + Y * Y + Z * Z); }
		}
		...
		{
			float scale = MathHelpers.InverseSqrtFast(X * X + Y * Y + Z * Z);
			X *= scale;
			Y *= scale;
			Z *= scale;
		}
		...
		{
			float scale = MathHelpers.InverseSqrtFast(vec.X * vec.X + vec.Y * vec.Y + vec.Z * vec.Z);
			vec.X *= scale;
			vec.Y *= scale;
			vec.Z *= scale;
			return vec;
