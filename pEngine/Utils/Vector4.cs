...
namespace pEngine
{
	...
	public struct Vector4 : IEquatable<Vector4>
	{
		...
		{
			get { return 1.0f / MathHelpers.InverseSqrtFast(X * X + Y * Y + Z * Z + W * W); }
		}
		...
		{
			float scale = MathHelpers.InverseSqrtFast(X * X + Y * Y + Z * Z + W * W);
			X *= scale;
			Y *= scale;
			Z *= scale;
			W *= scale;
		...
		{
			float scale = MathHelpers.InverseSqrtFast(vec.X * vec.X + vec.Y * vec.Y + vec.Z * vec.Z + vec.W * vec.W);
			vec.X *= scale;
			vec.Y *= scale;
			vec.Z *= scale;
			vec.W *= scale;
