...
namespace pEngine.Common.Math
{
	...
	public static class Interpolation
	{
		...
		public static Color4 ValueAt(double time, Color4 startColour, Color4 endColour, double startTime, double endTime, EasingTypes easing = EasingTypes.None)
		{
			...
			return new Color4(
				(float)Math.Max(0, Math.Min(1, ApplyEasing(easing, current, startColour.Rf, endColour.Rf - startColour.Rf, duration))),
				(float)Math.Max(0, Math.Min(1, ApplyEasing(easing, current, startColour.Gf, endColour.Gf - startColour.Gf, duration))),
				(float)Math.Max(0, Math.Min(1, ApplyEasing(easing, current, startColour.Bf, endColour.Bf - startColour.Bf, duration))),
				(float)Math.Max(0, Math.Min(1, ApplyEasing(easing, current, startColour.Af, endColour.Af - startColour.Af, duration))));
		}
